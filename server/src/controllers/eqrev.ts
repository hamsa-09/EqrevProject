    ...
    try {
        ...
        for (const row of rows) {
            ...
            factRows.push({
                ...
                stock_at_warehouses: headers['stock_at_warehouses']
                    ...
                    : 0,
                ad_spend: headers['ad_spend']
                    ? parseFloat(headers['ad_spend'])
                    : 0,
                ...
                ad_add_to_carts: headers['ad_add_to_carts']
                    ...
                    : 0,
                ad_orders: headers['ad_orders']
                    ? parseInt(headers['ad_orders'])
                    : 0,
        ...
        const productsData = Array.from(productSet.values())
            .map((p) => {
                ...
                const { brand_name, category_name, subcategory_name, ...rest } =
                    p;
...
export const dashboardMetric = async (req: Request, res: Response) => {
    try {
        ...
        if (customStart && customEnd) {
            ...
            // Validate same range between dates
            const { valid, message } = validateDateRangeMatch(
                startDate,
                endDate,
                cs,
                ce
        ...
        // Fetch metrics for current and comparison periods in parallel
            ...
            (current: DBCategoryMetric) => {
                ...
                return {
                    ...
                    totalRevenueDiff:
                        ...
                        (Number(compare?.totalRevenue) || 0),
                    totalOrders: Number(current.totalOrders) || 0,
                    totalOrdersDiff:
                        (Number(current.totalOrders) || 0) -
                        (Number(compare?.totalOrders) || 0),
                    adSpends: Number(current.adSpends) || 0,
                    adSpendsDiff:
                        (Number(current.adSpends) || 0) -
                        (Number(compare?.adSpends) || 0),
                    adRevenue: Number(current.adRevenue) || 0,
                    adRevenueDiff:
                        (Number(current.adRevenue) || 0) -
                        (Number(compare?.adRevenue) || 0),
                    roas: Number(current.roas) || 0,
                    roasDiff:
                        (Number(current.roas) || 0) -
                        (Number(compare?.roas) || 0),
                    aov: Number(current.aov) || 0,
                    aovDiff:
                        (Number(current.aov) || 0) -
                        (Number(compare?.aov) || 0),
                };
            }
        );

        // default sorting  by totalRevenue
 ...
 * @param {string} metric1 - 'totalRevenue'
 * @param {string} metric2 - 'totalOrders'
 */
export const getLineChartMetrics = async (req: Request, res: Response) => {
    try {
        ...
        // Fetch daily metrics
        const rawData = await getDailyMetrics(
            startDate,
            endDate,
            firstMetric,
            secondMetric
